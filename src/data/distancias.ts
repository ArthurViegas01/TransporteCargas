// Fonte: DNIT — Distâncias rodoviárias entre capitais brasileiras (km)
export const CIDADES = [
  'ARACAJU', 'BELÉM', 'BELO HORIZONTE', 'BRASÍLIA', 'CAMPO GRANDE',
  'CUIABÁ', 'CURITIBA', 'FLORIANÓPOLIS', 'FORTALEZA', 'GOIÂNIA',
  'JOÃO PESSOA', 'MACEIÓ', 'MANAUS', 'NATAL', 'PORTO ALEGRE',
...
// Chaves normalizadas (sem acento, uppercase) para compatibilidade
export const CIDADES_NORMALIZED: Record<string, Cidade> = {
  'ARACAJU': 'ARACAJU',
  'BELEM': 'BELÉM',
  'BELÉM': 'BELÉM',
...
// Matriz de distâncias — índices seguem a ordem de CIDADES
export const MATRIZ_DISTANCIAS: number[][] = [
  //ARA    BEL   BHZ   BSB   CGR   CUI   CWB   FLN   FOR   GOI   JPA   MCE   MAO   NAT   POA   PVH   REC   RBR   RIO   SAL   SLZ   SAO   THE   VIT
...
/** Retorna a distância em km entre duas cidades (aceita com ou sem acento, uppercase ou não). */
export function getDistancia(cidadeA: string, cidadeB: string): number | null {
  const a = CIDADES_NORMALIZED[cidadeA.toUpperCase().trim()];
  const b = CIDADES_NORMALIZED[cidadeB.toUpperCase().trim()];
  if (!a || !b) return null;
  const iA = CIDADES.indexOf(a);
  const iB = CIDADES.indexOf(b);
  if (iA === -1 || iB === -1) return null;
  return MATRIZ_DISTANCIAS[iA][iB];
}
